...
```{r}
gwasData <- read.table("~/Bioinfo/TFG/data/daner_PGC_BIP32b_mds7a_0416a", header = TRUE) 
...
nCHRgwas <- length(unique(gwasData$CHR))
gwasData$BPcum <- NA
s <- 0
nbpg <- c()
for (i in 1:22){
  nbpg[i] <- max(gwasData[gwasData$CHR == i,]$BP)
...
```{r}
ggplot(gwasData, aes(BPcum,-log10(pval))) + geom_point(color = "grey") +
  geom_point(data = PascalData, aes(BPcum,-log10(pval), color = col)) +
  ...
  geom_label_repel( data=highlight_df, aes(label=gene_symbol), size=2, max.overlaps = Inf)

#ggsave("~/Bioinfo/TFG/results/plots/BD1_gwas.png", height = 6, width = 12)
```
